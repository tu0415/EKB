	...
	<view class="register">
		...
		<view class="cont ml30 mr30">
			...
			<view class="f28 bg062B37 br10 item flexAI mt20">
				...
				<input class="f28" type="password"  v-model.trim="password" placeholder="请设置登录密码" />
			</view>
			<view class="f28 bg062B37 br10 item flexAI mt20">
				<text class="flexAI">新密码</text>
				<input class="f28" type="password"  v-model.trim="newpasssword" placeholder="请确认登录密码" />
			</view>
	...
	export default {
		...
		methods:{
			...
			changpasswordEvt() {
				...
				this.$http.quest(this.$API.login.forgetpss, "post", {
					tel: this.phone,
					pwd: this.password,
					pwdQr: this.newpasssword,
					card: this.code,
